...
<div class="widget-container" id="group_activity_widget">
	...
	<ul class="thumblist">
		...
		<li>
			...
			</h4> <span><?php echo $activity->description ?> </span> <a
			class="read" href="/event/<?php echo $activity->id ?>">Read More
				&#187;</a>
		</li>
		<?php
		}
	...
	<ul class="extralist">
		...
		for ($i=2; $i<(count($this->events)<6? count($this->events) : 6); $i++) {
			$activity = $$this->events[$i];
			?>
		<li><a href="/event/<?php echo $activity->id ?>"><?php echo $activity->title ?>
		</a></li>
		<?php
		}
	}
