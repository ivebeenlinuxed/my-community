...
?><div class="widget-container" id="group_news_widget">
				...
				for ($i=0; $i<(count($this->articles)<2? count($this->articles) : 2); $i++) {
					...
					<span><?php echo $news->text ?> </span> <a class="read"
						href="<?php echo $news->link ?>">Read More &#187;</a></li>
						<?php
				}
				?>
				</ul>
			...
			if (count($this->articles) > 2) {
				...
				<?php 
					...
					?>
					<label for="rss_off" class="radio"><input type="radio" value="OFF" name="rss" id="rss_off"<?php if ($this->rss == "") {echo "checked ";} ?> />Disable RSS</label><br />
					<label for="rss_on" class="radio"><input type="radio" value="ON" name="rss" id="rss_on"<?php if ($this->rss != "") {echo "checked ";} ?> /><input type="text" name="rss_feed" value="<?php echo $this->rss ?>" placeholder="RSS Feed" /></label><br />
